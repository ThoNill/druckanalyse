...
public class AbfrageRegisterTest {
    ...
    public void test3() {
        ...
        assertEquals(1, register.getWurzelAbfragen().size());
        assertTrue(cset.contains(new Abfrage("ab1",
                "select name , gebdatum, ort from kunde")));
        assertTrue(cset
                .contains(new Abfrage("ab2", "select  ort from adresse ")));

        int countFound = 0;
        for (Abfrage a : cset) {
            if ("ab1".equals(a.getName())) {
                countFound++;
                FeldReihe child = a.getChilds().get(0);
